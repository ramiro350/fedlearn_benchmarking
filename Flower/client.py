...
class DataClient(fl.client.NumPyClient):
    ...
    def fit(
        ...
        net.train(self.model, self.trainloader, epochs=1, device=DEVICE)
    ...
    def evaluate(
        ...
        self.set_parameters(parameters)
        loss, accuracy = net.test(self.model, self.testloader, device=DEVICE)
...
def main() -> None:
    ...
    # Load data
    trainloader, testloader = net.load_data(args.partition_id)

    # Load model
    model = net.Net().to(DEVICE).train()
